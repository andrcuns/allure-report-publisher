...
export class GitlabCiProvider extends BaseCiProvider {
  ...
  private readonly mrIid = this.ciInfo.mrIid
  ...
  private async updateMrDescription() {
    if (!this.mrIid) throw new Error('Could not detect merge request iid!')
    ...
    await this.client.MergeRequests.edit(this.ciInfo.projectId!, this.mrIid, {
  ...
  private async updateComment() {
    ...
    const response = comment
      ? await this.client.MergeRequestNotes.edit(this.ciInfo.projectId!, this.mrIid, comment.id, {
          body: updatedComment,
        })
      : await this.client.MergeRequestNotes.create(this.ciInfo.projectId!, this.mrIid, updatedComment)
    logger.debug(`MR comment with id '${response.id}' ${comment ? 'updated' : 'created'} successfully`)
  }

  private async getMrDescription() {
    logger.debug(`Fetching MR description for mr '${this.mrIid}'`)
    const mr = await this.client.MergeRequests.show(this.ciInfo.projectId!, this.mrIid!)
    logger.debug('Fetched MR description')
    return mr.description || ''
  }

  private async getComment() {
    logger.debug(`Fetching MR comment for mr '${this.mrIid}'`)
    const comments = await this.client.MergeRequestNotes.all(this.ciInfo.projectId!, this.mrIid!, {
      sort: 'asc',
      orderBy: 'created_at',
      perPage: 100,
    })
